...
if [ "$EUID" -ne 0 ]; then 
  echo "Rquires Root: sudo ./db-firewall.sh"
  exit
fi

# Variables
INTERFACE="en0" # Replace with Interface of server
DB_SCORING_IP="0.0.0.0" # Replace with DB scoring IP if found
EXTERNAL_KALI="192.168.24.20" # Replace with IP of any Kali machine used for remote access
WEB_SERVER="172.50.120.20" # Replace with Web Server IP
INTERNET="0.0.0.0" # Used to access internet for downloading
DNS_SERVER="0.0.0.0" # Use if needed

# Path to command binaries
# Still confirm the itegrity of these binaries and confirm with "which" command
...
# DB Scoring Zone
$FIREWALL_CMD --permanent --delete-zone=db_scoring >/dev/null 2>&1
$FIREWALL_CMD --permanent --new-zone=db_scoring
$FIREWALL_CMD --permanent --zone=db_scoring --add-source=${DB_SCORING_IP}
$FIREWALL_CMD --permanent --zone=db_scoring --add-service=postgresql
$FIREWALL_CMD --permanent --zone=db_scoring --set-target=ACCEPT

# Web Zone
$FIREWALL_CMD --permanent --delete-zone=db_web >/dev/null 2>&1
$FIREWALL_CMD --permanent --new-zone=db_web
$FIREWALL_CMD --permanent --zone=db_web --add-source=${WEB_SERVER}
$FIREWALL_CMD --permanent --zone=db_web --add-service=postgresql
$FIREWALL_CMD --permanent --zone=db_web --set-target=ACCEPT
